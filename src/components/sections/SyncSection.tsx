...
const ROWS = [
  { name: "Strava", logo: strava, protocol: "API v3", direction: "Two-way", latency: "< 60 s" },
  { name: "Garmin", logo: garmin, protocol: "FIT · Connect", direction: "Two-way", latency: "< 30 s" },
  { name: "TrainingPeaks", logo: trainingpeaks, protocol: "WKO · TCX", direction: "Two-way", latency: "< 60 s" },
  { name: "COROS", logo: coros, protocol: "FIT · EvoLab", direction: "Inbound", latency: "< 30 s" },
];
  ...
  useGSAP(
    () => {
      ...
      mm.add("(prefers-reduced-motion: no-preference)", () => {
        gsap.utils.toArray<HTMLElement>(".sync-row").forEach((row, i) => {
          const tl = gsap.timeline({
            scrollTrigger: { trigger: row, start: "top 90%" },
            delay: i * 0.06,
          });
          tl.from(row.querySelector(".sync-rule"), {
            scaleX: 0,
            transformOrigin: "left center",
            duration: 0.9,
            ease: "power3.inOut",
          }).from(
            row.querySelector(".sync-content"),
